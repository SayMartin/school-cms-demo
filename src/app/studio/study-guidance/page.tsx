...
export default function StudioStudievagledningPage() {
 ...
 useEffect(() => {
 fetch("/api/study-guidance/content")
 .then((r) => r.json() as Promise<{ blocks: string; sidebarProfileIds: string; heading: string; headingVisible: boolean; headingColor?: string }>)
 .then((d) => {
 ...
 setError(null);
 try {
 const res = await fetch("/api/study-guidance/content", {
 method: "PUT",
 ...
 <h1 className="mt-1">Study Guidance</h1>
 ...
 <BlockToolbar types={["section", "accordion-section", "slideshow", "profiles", "youtube", "video"]} onAdd={addBlock} />
 </StudioSaveBar>
 ...
 <SlideshowBlockEditor block={block} uploadPrefix="studievagledning" onChange={(patch) => updateBlock(block.id, patch)} />
 )}
 {block.type === "youtube" && (
