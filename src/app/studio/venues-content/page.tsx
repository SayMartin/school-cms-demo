...
export default function VenuesStudioPage() {
  ...
  useEffect(() => {
    fetch("/api/venues-content/content")
  ...
  async function doSave() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/venues-content/content", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ blocks, heading, headingVisible, headingColor }),
      });
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12">
      ...
      <h1 className="mt-1">Venues</h1>

      <form onSubmit={(e) => { e.preventDefault(); void doSave(); }} className="mt-8">
        <StudioSaveBar
          isDirty={isDirty}
          saving={saving}
          error={error}
        ...
        >
          <BlockToolbar
            types={["nav-group", "course-group", "section", "accordion-section", "slideshow", "profiles", "youtube", "video"]}
            onAdd={addBlock}
          />
        </StudioSaveBar>
        ...
        {blocks.length === 0 ? (
          <div className="mt-6 rounded-lg border border-dashed border-gray-300 bg-gray-50 px-4 py-8 text-center">
            <p className="text-sm text-gray-600">No blocks. Add a block above.</p>
          </div>
        ) : (
            ...
            {blocks.map((block, idx) => (
              ...
              >
                {block.type === "nav-group" && (
                  <NavGroupBlockEditor block={block} onChange={(patch) => updateBlock(block.id, patch)} uploadPrefix="venues" />
                )}
                {block.type === "section" && (
                ...
                {block.type === "slideshow" && (
                  <SlideshowBlockEditor block={block} uploadPrefix="venues" onChange={(patch) => updateBlock(block.id, patch)} />
                )}
                {block.type === "profiles" && (
