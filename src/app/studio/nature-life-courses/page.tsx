...
export default function StudioNaturlivskurserPage() {
  ...
  const isDirty = savedSnapshot !== "" && JSON.stringify({ blocks, heading, headingVisible, headingColor, imageKey }) !== savedSnapshot;

  useEffect(() => {
    fetch("/api/nature-life-courses/content")
      .then((r) => r.json() as Promise<{ blocks: string; heading: string; headingVisible: boolean; headingColor?: string; imageKey?: string | null }>)
      .then((d) => {
        const parsed = parseHubBlocks(d.blocks ?? "[]");
        const h = d.heading ?? "";
        const hv = d.headingVisible ?? true;
        const hc = d.headingColor;
        const ik = d.imageKey ?? null;
        setBlocks(parsed);
        setHeading(h);
        setHeadingVisible(hv);
        setHeadingColor(hc);
        setImageKey(ik);
        setSavedSnapshot(JSON.stringify({ blocks: parsed, heading: h, headingVisible: hv, headingColor: hc, imageKey: ik }));
      })
  ...
  function updateBlock(id: string, patch: Partial<HubBlock>) {
    setBlocks(blocks.map((b) => b.id === id ? { ...b, ...patch } as HubBlock : b));
  }
  ...
  function doDiscard() {
    const snap = JSON.parse(savedSnapshot) as { blocks: HubBlock[]; heading: string; headingVisible: boolean; headingColor?: string; imageKey: string | null };
  ...
  async function doSave() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/nature-life-courses/content", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ blocks: JSON.stringify(blocks), heading, headingVisible, headingColor, imageKey }),
      });
      if (!res.ok) throw new Error(await res.text());
      setSavedSnapshot(JSON.stringify({ blocks, heading, headingVisible, headingColor, imageKey }));
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12">
      ...
      <h1 className="mt-1">Nature & Life Courses — hub page</h1>
      ...
      <form onSubmit={(e) => { e.preventDefault(); void doSave(); }} className="mt-8 space-y-6">
        <StudioSaveBar isDirty={isDirty} saving={saving} error={error} onSave={doSave} onDiscard={doDiscard}>
          <BlockToolbar
            types={["nav-group", "course-group", "section", "accordion-section", "slideshow", "youtube", "video"]}
            onAdd={addBlock}
          />
        </StudioSaveBar>

        <div className="space-y-1">
          <label className="text-sm font-medium text-gray-700">Hero image <span className="font-normal text-gray-600">(full width, optional)</span></label>
          <ImageUpload value={imageKey} prefix="naturlivskurser" onChange={setImageKey} />
        </div>

        <div className="space-y-1">
          <label className="text-sm font-medium text-gray-700">Page heading <span className="font-normal text-gray-600">(H1, left)</span></label>
          ...
          {blocks.map((block, idx) => (
            ...
            >
              ...
              {block.type === "slideshow" && (
                <SlideshowBlockEditor block={block} uploadPrefix="naturlivskurser" onChange={(patch) => updateBlock(block.id, patch)} />
              )}
              {block.type === "youtube" && (
              ...
              {block.type === "nav-group" && (
                <NavGroupBlockEditor block={block} onChange={(patch) => updateBlock(block.id, patch)} />
              )}
              {block.type === "course-group" && (
                <KursgruppBlockEditor block={block} onChange={(patch) => updateBlock(block.id, patch)} />
              )}
            </BlockCard>
          ))}
