...
export default function StudioStuderanderattPage() {
 ...
 useEffect(() => {
 fetch("/api/student-rights/content")
 ...
 function doDiscard() {
 const snap = JSON.parse(savedSnapshot) as { blocks: ContentBlock[]; heading: string; headingVisible: boolean; headingColor?: string };
 ...
 setError(null);
 try {
 const res = await fetch("/api/student-rights/content", {
 method: "PUT",
 ...
 <h1 className="mt-1">Student Rights</h1>
 ...
 <label className="text-sm font-medium text-gray-700">Page heading <span className="font-normal text-gray-600">(H1 centered)</span></label>
 ...
 <p className="text-sm text-gray-600">The policy document links at the bottom of the page are edited by a developer. Include PDF links in the body text using the link tool.</p>
 ...
 <SlideshowBlockEditor block={block} uploadPrefix="studeranderatt" onChange={(patch) => updateBlock(block.id, patch)} />
 )}
 {block.type === "youtube" && (
