...
export default function StudioPraktiskInfoSommarkurserPage() {
 ...
 useEffect(() => {
 fetch("/api/summer-courses-practical-info/content")
 ...
 setError(null);
 try {
 const res = await fetch("/api/summer-courses-practical-info/content", {
 method: "PUT",
 ...
 <h1 className="mt-1">Summer Courses Practical Info</h1>
 ...
 <SlideshowBlockEditor block={block} uploadPrefix="praktisk-info" onChange={(patch) => updateBlock(block.id, patch)} />
 )}
 {block.type === "youtube" && (
 ...
 No blocks yet — add one with the toolbar above.
 </p>
 )}
 </div>
 </form>
