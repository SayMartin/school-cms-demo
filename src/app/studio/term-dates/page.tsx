...
export default function StudioLovtiderPage() {
 ...
 setError(null);
 try {
 const res = await fetch("/api/term-dates/content", {
 method: "PUT",
 ...
 <h1 className="mt-1">Term Dates</h1>
 ...
 <p className="text-sm text-gray-600">Use H2 for the term name (e.g. Spring 2026) and a list with event — date per row. Evening courses and short courses have their own schedules.</p>
 ...
 <SlideshowBlockEditor block={block} uploadPrefix="lovtider" onChange={(patch) => updateBlock(block.id, patch)} />
 )}
 {block.type === "youtube" && (
