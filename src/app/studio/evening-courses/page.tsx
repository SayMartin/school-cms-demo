...
export default function StudioKvallskurserContentPage() {
  ...
  useEffect(() => {
    fetch("/api/evening-courses/content")
      .then((r) => r.json() as Promise<{ blocks: string; heading: string; headingVisible: boolean; headingColor?: string }>)
      .then((d) => {
  ...
  function doDiscard() {
    const snap = JSON.parse(savedSnapshot) as { blocks: HubBlock[]; heading: string; headingVisible: boolean; headingColor?: string };
  ...
  async function doSave() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/evening-courses/content", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ blocks: JSON.stringify(blocks), heading, headingVisible, headingColor }),
      });
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12">
      ...
      <h1 className="mt-1">Evening Courses — hub page</h1>
      ...
      <form onSubmit={(e) => { e.preventDefault(); void doSave(); }} className="mt-8 space-y-6">
        <StudioSaveBar isDirty={isDirty} saving={saving} error={error} onSave={doSave} onDiscard={doDiscard}>
          <BlockToolbar types={["nav-group", "course-group", "section", "accordion-section", "slideshow", "youtube", "video"]} onAdd={addBlock} />
        </StudioSaveBar>
        ...
        <div className="space-y-3">
          <p className="text-sm text-gray-600">
            <strong>CourseGroup</strong> blocks control which courses are shown and in what order. Other blocks appear as editorial content above the course groups.
          </p>

          {blocks.map((block, idx) => (
            <BlockCard key={block.id} label={BLOCK_LABELS[block.type]}
              summary={block.type === "accordion-section" ? block.summary : block.heading}
              summaryColor={block.type === "section" ? block.headingColor : undefined}
            ...
            >
              ...
              {block.type === "slideshow" && (
                <SlideshowBlockEditor block={block} uploadPrefix="kvallskurser" onChange={(patch) => updateBlock(block.id, patch)} />
              )}
              {block.type === "youtube" && (
