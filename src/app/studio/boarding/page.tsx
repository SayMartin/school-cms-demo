...
export default function StudioInternatPage() {
 ...
 setError(null);
 try {
 const res = await fetch("/api/boarding/content", {
 method: "PUT",
 ...
 <div>
 <h2 className="text-base">Page content</h2>
 <p className="mt-0.5 text-sm text-gray-600">The page heading is always shown at the top. Add and arrange blocks freely below.</p>
 </div>

 {blocks.length === 0 && (
 <p className="rounded-md border border-dashed border-gray-200 px-4 py-8 text-center text-sm text-gray-600">
 ...
 <SlideshowBlockEditor block={block} uploadPrefix="internat" onChange={(patch) => updateBlock(block.id, patch)} />
 )}
