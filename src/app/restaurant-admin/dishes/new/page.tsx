...
export default function NewRattPage() {
  ...
  async function doSave() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/restaurant/dishes", {
        method: "POST",
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12">
      ...
      <h1 className="mt-1 text-gray-900">New Dish</h1>
      ...
      <form onSubmit={(e) => { e.preventDefault(); void doSave(); }} className="mt-8 space-y-6">
        ...
        <div>
          <label htmlFor="allergens" className="block text-base font-medium text-gray-700">Allergens</label>
          <RichTextEditor value={form.allergens} onChange={(html) => setForm((f) => ({ ...f, allergens: html }))} placeholder="e.g. Gluten, Milk, Egg" />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="price" className="block text-base font-medium text-gray-700">Price (kr)</label>
          ...
          <div>
            <label htmlFor="studentPrice" className="block text-base font-medium text-gray-700">Student Price (kr)</label>
