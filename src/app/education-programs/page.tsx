...
export const metadata: Metadata = { title: "Utbildningar" };
export const dynamic = "force-dynamic";

export default async function UtbildningarPage() {
  const db = getDb();

  const row = await db.select().from(navHubContent).where(eq(navHubContent.id, "utbildningar")).limit(1).then((r) => r[0]);

  const heading = row?.heading || "Utbildningar";
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 space-y-8">
      <div className="text-center">
        ...
        {ingress && <p className="mt-3 max-w-7xl mx-auto text-gray-600">{ingress}</p>}
      </div>
