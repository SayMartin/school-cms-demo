...
const DEFAULT: Omit<typeof studentSupportContent.$inferInsert, "id" | "updatedAt"> = {
  sections: "[]",
  heading: "",
  headingVisible: true,
  headingColor: null,
...
export async function PUT(req: Request) {
  try {
    ...
    const body = await req.json() as Partial<Omit<typeof studentSupportContent.$inferInsert, "id" | "updatedAt">>;
    const db = getDb();
    const now = new Date();

    const [existing] = await db.select().from(studentSupportContent).where(eq(studentSupportContent.id, "main")).limit(1);

    if (existing) {
      const [updated] = await db
        .update(studentSupportContent)
        .set({ ...body, updatedAt: now })
        .where(eq(studentSupportContent.id, "main"))
        .returning();
      revalidatePath("/about/student-support");
      return NextResponse.json(updated);
    } else {
      const [created] = await db
        .insert(studentSupportContent)
