...
import { eveningCoursesContent } from "@/lib/db/schema";
...
export async function GET() {
  try {
    const db = getDb();
    const [row] = await db.select().from(eveningCoursesContent).where(eq(eveningCoursesContent.id, "main")).limit(1);
...
export async function PUT(req: Request) {
  try {
    ...
    const [existing] = await db.select().from(eveningCoursesContent).where(eq(eveningCoursesContent.id, "main")).limit(1);

    if (existing) {
      const [updated] = await db
        .update(eveningCoursesContent)
        ...
        .where(eq(eveningCoursesContent.id, "main"))
        .returning();
      revalidatePath("/evening-courses");
      return NextResponse.json(updated);
    } else {
      const [created] = await db
        .insert(eveningCoursesContent)
