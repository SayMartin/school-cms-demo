...
import { participantStoriesContent } from "@/lib/db/schema";
...
export async function GET() {
  try {
    const db = getDb();
    const [row] = await db.select().from(participantStoriesContent).where(eq(participantStoriesContent.id, "main")).limit(1);
...
export async function PUT(req: Request) {
  try {
    ...
    const [existing] = await db.select().from(participantStoriesContent).where(eq(participantStoriesContent.id, "main")).limit(1);

    if (existing) {
      const [updated] = await db
        .update(participantStoriesContent)
      ...
      const [created] = await db
        .insert(participantStoriesContent)
