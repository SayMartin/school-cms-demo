...
const DEFAULT: Omit<typeof historyContent.$inferInsert, "id" | "updatedAt"> = {
  sections: "[]",
  heading: "",
  headingVisible: true,
  headingColor: null,
...
export async function GET() {
  try {
    const db = getDb();
    const [row] = await db.select().from(historyContent).where(eq(historyContent.id, "main")).limit(1);
...
export async function PUT(req: Request) {
  try {
    ...
    const body = await req.json() as Partial<Omit<typeof historyContent.$inferInsert, "id" | "updatedAt">>;
    const db = getDb();
    const now = new Date();

    const [existing] = await db.select().from(historyContent).where(eq(historyContent.id, "main")).limit(1);
