...
import { careersContent } from "@/lib/db/schema";

export const dynamic = "force-dynamic";

const DEFAULT: Omit<typeof careersContent.$inferInsert, "id" | "updatedAt"> = {
  sections: "[]",
  heading: "",
  headingVisible: true,
  headingColor: null,
...
export async function GET() {
  try {
    const db = getDb();
    const [row] = await db.select().from(careersContent).where(eq(careersContent.id, "main")).limit(1);
...
export async function PUT(req: Request) {
  try {
    ...
    const body = await req.json() as Partial<Omit<typeof careersContent.$inferInsert, "id" | "updatedAt">>;
    const db = getDb();
    const now = new Date();

    const [existing] = await db.select().from(careersContent).where(eq(careersContent.id, "main")).limit(1);

    if (existing) {
      const [updated] = await db
        .update(careersContent)
        .set({ ...body, updatedAt: now })
        .where(eq(careersContent.id, "main"))
        .returning();
      revalidatePath("/about/careers");
      return NextResponse.json(updated);
    } else {
      const [created] = await db
        .insert(careersContent)
